...
const EventDetails = () => {
  ...
  useEffect(() => {
    if (event) {
      const basePrice = selectedType === 'general' ? event.price : event.vipPrice || event.price * 1.5;
  ...
  if (!event) return <div className="text-center py-20 text-xl">Loading...</div>;
  ...
  return (
    ...
    <div className="p-6 max-w-5xl mx-auto bg-black">
      <div className="rounded-xl overflow-hidden shadow-lg bg-white">
        ...
        <div className="p-6">
          <h2 className="text-4xl font-extrabold mb-2 text-gray-800">{event.name}</h2>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 text-base text-gray-600 mb-6">
            <p>💸 <strong>General Price:</strong> ₹{event.price}</p>
            <p>💎 <strong>VIP Price:</strong> ₹{event.vipPrice || event.price * 1.5}</p>
            <p>🎫 <strong>Tickets Left:</strong> {event.ticketsAvailable - selectedCount}</p>
          </div>

          <div className="grid grid-cols-1 sm:grid-cols-2 gap-6">
            <div>
              <label className="block mb-1 font-medium text-gray-700">🎟️ Ticket Type</label>
              <select
            ...
            <div>
              <label className="block mb-1 font-medium text-gray-700">🔢 Ticket Count</label>
              <select
                className="border border-gray-300 p-2 rounded w-full"
                value={selectedCount}
